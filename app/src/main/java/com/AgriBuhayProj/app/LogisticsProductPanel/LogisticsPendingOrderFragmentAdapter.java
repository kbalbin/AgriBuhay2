    ...
    public void onBindViewHolder(@NonNull LogisticsPendingOrderFragmentAdapter.ViewHolder holder, int position) {
        ...
        holder.Accept.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("LogisticsShipOrders").child(deliveryId).child(randomuid).child("Products");
                databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        DatabaseReference data = FirebaseDatabase.getInstance().getReference("LogisticsShipOrders").child(deliveryId).child(randomuid).child("OtherInformation");
                            ...
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                ...
                                FirebaseDatabase.getInstance().getReference("LogisticsShipFinalOrders").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(randomuid).child("OtherInformation").setValue(hashMap1).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        FirebaseDatabase.getInstance().getReference("LogisticsShipOrders").child(deliveryId).child(randomuid).child("Products").removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                            @Override
                                            public void onComplete(@NonNull Task<Void> task) {
                                                FirebaseDatabase.getInstance().getReference("LogisticsShipOrders").child(deliveryId).child(randomuid).child("OtherInformation").removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                                                    @Override
                        ...
                        FirebaseDatabase.getInstance().getReference().child("Tokens").child(producerid).child("token").addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                ...
                                FirebaseDatabase.getInstance().getReference("LogisticsShipOrders").child(deliveryId).child(randomuid).child("Products").removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        FirebaseDatabase.getInstance().getReference("LogisticsShipOrders").child(deliveryId).child(randomuid).child("OtherInformation").removeValue();
                                    }
                                });
                            }
